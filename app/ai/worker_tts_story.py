...
from app.consumers.tts_consumer import register_story_tts_consumer
...
# 워커 liveness 를 판정한다. 비즈니스 큐(ai.gpu.request.queue) 와 의도적으로 분리.
# 이름은 infra/scripts/health-check-{dev,master}.sh 의 ai-tts-story-worker 케이스와 동기화 필요.
...
def main() -> None:
    run_worker("ai-tts-story-worker", _register_consumers)
