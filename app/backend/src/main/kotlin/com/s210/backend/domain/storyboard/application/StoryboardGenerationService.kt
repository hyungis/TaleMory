 ...
 * 실행 흐름:
 *  1. 소유권 검증
 *  2. Story 기본 필드(여행지 등) 검증
 *  3. 사진 목록 로드 — 최소 1장 필수
 *  4. Story 의 JSON 문자열 필드(mainCharacterJson / companionsJson)를 AI 가 요구하는 구조로 파싱
 *  5. `story_generation_jobs` 에 PENDING row 생성 — external_id = UUID v4
 *  6. envelope 를 ai.request exchange 로 publish (routing key = ai.cpu.story.generate)
 *  7. `{jobId, status}` 즉시 응답
 *
 * AI 워커는 메시지를 받고 OpenAI 호출 후 ai.result exchange 로 응답을 publish 한다.
 * Spring 측 결과 수신은 후속 Listener (Phase 5) 에서 처리.
 */
@Service
@Transactional
class StoryboardGenerationService(
    ...
    fun generate(userId: Long, storyId: Long): StartGenerationResult {
        ...
        if (photos.isEmpty()) {
            // Step 2 에서 사진을 1장 이상 업로드해야 생성 가능.
            throw BusinessException(CommonErrorCode.INVALID_INPUT)
        }
